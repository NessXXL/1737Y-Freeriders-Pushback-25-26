...
float freeriders::slew(float target, float current, float maxChange) {
...
constexpr float freeriders::sanitizeAngle(float angle, bool radians) {
    if (radians) return std::fmod(std::fmod(angle, 2 * M_PI) + 2 * M_PI, 2 * M_PI);
    else return std::fmod(std::fmod(angle, 360) + 360, 360);
}

float freeriders::angleError(float target, float position, bool radians, AngularDirection direction) {
...
float freeriders::avg(std::vector<float> values) {
    float sum = 0;
    for (float value : values) { sum += value; }
    return sum / values.size();
}

float freeriders::ema(float current, float previous, float smooth) {
    return (current * smooth) + (previous * (1 - smooth));
}

float freeriders::getCurvature(Pose pose, Pose other) {
    // calculate whether the pose is on the left or right side of the circle
    float side = freeriders::sgn(std::sin(pose.theta) * (other.x - pose.x) - std::cos(pose.theta) * (other.y - pose.y));
