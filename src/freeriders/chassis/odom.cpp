...
// global variables
freeriders::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
freeriders::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
freeriders::Pose odomPose(0, 0, 0); // the pose of the robot
freeriders::Pose odomSpeed(0, 0, 0); // the speed of the robot
freeriders::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void freeriders::setSensors(freeriders::OdomSensors sensors, freeriders::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

freeriders::Pose freeriders::getPose(bool radians) {
    if (radians) return odomPose;
    else return freeriders::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void freeriders::setPose(freeriders::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = freeriders::Pose(pose.x, pose.y, degToRad(pose.theta));
}

freeriders::Pose freeriders::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return freeriders::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

freeriders::Pose freeriders::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return freeriders::Pose(odomLocalSpeed.x, odomLocalSpeed.y, radToDeg(odomLocalSpeed.theta));
}

freeriders::Pose freeriders::estimatePose(float time, bool radians) {
...
void freeriders::update() {
    ...
    freeriders::TrackingWheel* verticalWheel = nullptr;
    freeriders::TrackingWheel* horizontalWheel = nullptr;
    ...
    // save previous pose
    freeriders::Pose prevPose = odomPose;
