 ...
 * @return std::vector<freeriders::Pose> vector of points on the path
 */
std::vector<freeriders::Pose> getData(const asset& path) {
    std::vector<freeriders::Pose> robotPath;
    ...
    for (std::string line : dataLines) {
        freeriders::infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            freeriders::infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        freeriders::Pose pathPoint(0, 0);
        ...
        freeriders::infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
int findClosest(freeriders::Pose pose, std::vector<freeriders::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(freeriders::Pose p1, freeriders::Pose p2, freeriders::Pose pose, float lookaheadDist) {
    // calculations
    // uses the quadratic formula to calculate intersection points
    freeriders::Pose d = p2 - p1;
    freeriders::Pose f = p1 - pose;
    float a = d * d;
    float b = 2 * (f * d);
 ...
 */
freeriders::Pose lookaheadPoint(freeriders::Pose lastLookahead, freeriders::Pose pose, std::vector<freeriders::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        freeriders::Pose lastPathPose = path.at(i);
        freeriders::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            freeriders::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(freeriders::Pose pose, float heading, freeriders::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = freeriders::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void freeriders::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<freeriders::Pose> pathPoints = getData(path); // get list of path points
